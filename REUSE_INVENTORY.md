...
| `packages/case-writer/src/openAiCaseWriterClient.js`, `src/config/modelConfig.js`, `config/model.json` | Added GPT-5.6-first compatibility negotiation for API projects with different model access and sanitized upstream errors so project metadata never reaches the reviewer UI. |
